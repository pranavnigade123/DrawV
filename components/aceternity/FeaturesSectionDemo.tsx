...
const FeatureTitle = ({ children }: { children?: React.ReactNode }) => {
  return (
    <p className="max-w-5xl mx-auto text-center tracking-tight text-white text-xl md:text-2xl md:leading-snug px-8">
      {children}
    </p>
  );
};
    ...
    <div className="relative flex flex-col items-center py-8 px-4 gap-6 h-full">
      <div className="w-full flex justify-center mb-4">
        <img
          ...
          width={1200}
          height={700}
          className="w-[100%] max-w-2xl rounded-xl shadow-lg object-contain bg-neutral-800 mx-auto"
        />
      </div>
      <div className="w-full flex flex-col items-center text-center mt-2">
        <p className="text-pretty text-base md:text-lg text-neutral-200 leading-relaxed px-4 mt-12 text-justify">
      ...
      <div className="absolute bottom-0 z-40 inset-x-0 h-40 bg-gradient-to-t from-black/60 via-black/20 to-transparent w-full pointer-events-none" />
      <div className="absolute top-0 z-40 inset-x-0 h-24 bg-gradient-to-b from-black/30 via-transparent to-transparent w-full pointer-events-none" />
    </div>
  );
};

export const SkeletonTwo = () => {
  const images = ["aarohan2.jpg", "aarohan3.jpg", "aarohan6.jpg"];
  return (
    <div className="flex flex-col items-center gap-4 py-4 px-2 w-full h-full">
      {images.map((src, idx) => (
        <img
          ...
          className="w-[96%] max-w-3xl rounded-xl object-cover shadow-md bg-neutral-800"
        />
      ))}
    </div>
  );
