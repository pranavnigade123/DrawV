    ...
    <div className="mb-5 rounded-2xl border border-zinc-800/70 bg-zinc-900/60 backdrop-blur supports-[backdrop-filter]:bg-zinc-900/40">
      <div className="px-4 sm:px-5 py-3 flex items-center justify-between gap-3">
        ...
        <div className="min-w-0 flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="text-[11px] uppercase tracking-wide text-zinc-400 truncate">
              {crumbs.map((c, i) => (
                <span key={c.href}>
                  {i > 0 && <span className="mx-1.5 text-zinc-500">›</span>}
                  <Link
                    href={c.href}
                    className={
                      i === crumbs.length - 1
                        ? 'text-zinc-400 cursor-default pointer-events-none'
                        : 'hover:underline'
                    }
                  >
                    {c.label}
            ...
            {title && (
              <div className="mt-0.5 text-lg font-semibold truncate text-white">
                {title}
              </div>
            )}
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href="/"
            className="px-3 py-1.5 rounded-lg border border-zinc-700 text-sm hover:bg-zinc-800 transition text-white"
            title="Go to site"
          >
            Site
          </Link>
